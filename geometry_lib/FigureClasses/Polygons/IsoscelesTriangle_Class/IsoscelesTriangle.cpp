...
IsoscelesTriangle::IsoscelesTriangle(const sf::Color& col, double A_size, double B_size, double C_size,std::string name)
        :Triangle(col, A_size, B_size, C_size, std::move(name)) {}

bool IsoscelesTriangle::is_possible_draw() {
    ...
    bool condition5 = (A_size == B_size) || (A_size == C_size) || (B_size == C_size);
